...
// export async function getAllEvents (req:Request, res:Response<Event[] | {message: string}>) {
//         try {
//             const result = await pool.query(`${eventsSelectQuery} ${eventsGroupQuery}`)
//             res.json(result.rows)

//         } catch (err) { 

//             res.status(500).json({message: `Failed to get events: ${err}`})
//         } 
//     }
